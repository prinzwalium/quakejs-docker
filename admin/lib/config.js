...
const FIELDS = {
  ...
  // Admin-only settings (no game cvar)
  nameMode:      { type: 'int', enum: [0, 1], def: 0 },  // 0: any name, 1: roster names only (enforced)
  statsPublic:   { type: 'bool', def: false },
};
...
function extraLines(extra) {
  return extra.split('\n')
    ...
    .filter(l => l && !/rconpassword|g_log|\bvstr\s+d\d|\bset[as]?\s+d\d+\b|\bquit\b|\bexec\b/i.test(l));
}
...
function renderSettingsCfg(s, rconPassword) {
  ...
  // Game log for player statistics; flushed after every line so the admin sees it immediately.
  lines.push('seta g_log "games.log"', 'seta g_logSync 1');
...
module.exports = { FIELDS, GAMETYPES, MAP_RE, defaults, validate, ValidationError, renderSettingsCfg, cleanString, writeFileAtomic, Store };
